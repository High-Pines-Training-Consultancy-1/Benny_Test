...
(error, record, runtime, search, render) => {
  ...
  const each = (params) => {
    try {
      ...
      pdfFile.folder = 100359; // Replace with your folder internal ID
      pdfFile.name = `Invoice_${docNumber}.pdf`; // Use document number
      let fileId = pdfFile.save();

      log.debug("PDF Saved", `Invoice: ${docNumber}, File ID: ${fileId}`);
    } catch (e) {
      log.error(
